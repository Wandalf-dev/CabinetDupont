...
class ActusController {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                // SÉCURITÉ 2 : Vérifier le type MIME réel
                ...
                // SÉCURITÉ 4 : Générer un nom de fichier sécurisé
                $fileName = \App\Core\Security::generateSecureFilename($originalFileName);
                $uploadFile = $uploadDir . $fileName;
                
                // Upload du fichier
                if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadFile)) {
                    chmod($uploadFile, 0644); // Permissions sécurisées
                    $data['image'] = $fileName;
                } else {
    ...
    public function search() {
        ...
        $keyword = isset($_GET['q']) ? htmlspecialchars(strip_tags($_GET['q']), ENT_QUOTES, 'UTF-8') : '';
        if ($keyword) {
